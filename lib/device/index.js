...
/**
 * Push orangutan binary to device
 * @returns {Promise}
 */
...
/**
 * Get the device's Gaia revision and cache the value with the device
 * @returns {Promise}
 */
...
/**
 * Get the device's Gecko revision and cache the value with the device
 * @returns {Promise}
 */
...
/**
 * adb forward with an optional port
 * @param [number] local defaults to 2828
 * @param [number] remote defaults to 2828
 * @returns {Promise}
 */
Device.prototype.adbForward = function(local, remote) {
  if (this.options.marionetteHost !== 'localhost') {
    return Promise.resolve();
  }

  return this.command()
    .env('ANDROID_SERIAL', this.serial)
    .adbForward(local || 2828, remote || 2828)
    .exec();
};
...
Device.create = function(serial, options) {
  /**
   * A complete device connection has the following workflow:
   *
   * 1. Instantiate a Device API
   * 2. Get the device matching the given serial, or the first device available
   * 3. Fetch the devices system properties
   * 4. Set the device-specific input configuration
   * 5. Set up major Device APIs
   * 6. Push the input triggering binary to the device
   * 7. Capture Gaia and Gecko revisions
   */
